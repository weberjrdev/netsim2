...
set tracefd	[open tr-files/shadowing_indoor_opaque_rand.tr w]
...
set namtrace [open nam-files/shadowing_indoor_opaque_rand.nam w]
$ns_ namtrace-all-wireless $namtrace $val(x) $val(y)

set prop [new $val(shadprop)]

$prop set pathlossExp_ 4.0
$prop set std_db_ 7.0
$prop set dist0_ 100.0
$prop seed raw 5124
...
for {set i 0} {$i < $val(stop)} {incr i} {
	set x_rand [expr { 50+round(rand()*400) }] 
	set y_rand [expr { 300+round(rand()*400) }]
	
	$ns_ at $i "$node_(1) setdest $x_rand $y_rand 1000.0"
}

set x_rand [expr { 150+round(rand()*100) }] 
set y_rand [expr { 400+round(rand()*100) }] 
